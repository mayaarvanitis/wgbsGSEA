...
```
```{r}
rownames(beta_matrix) <- trimws(rownames(beta_matrix))
custom_mapping <- trimws(custom_mapping)
# Trim any leading/trailing whitespaces from both sets
rownames(beta_matrix) <- trimws(rownames(beta_matrix))
names(custom_mapping) <- trimws(names(custom_mapping))
...
```
```{r}
# Create a data frame from the beta matrix
beta_matrix_with_genes <- data.frame(
  cpg_id = rownames(beta_matrix_valid),  # CpG IDs
  beta_matrix_valid                      # Beta values for each CpG
)

# Assuming `custom_mapping_valid` maps CpG IDs to gene names
beta_matrix_with_genes$gene_name <- custom_mapping_valid[beta_matrix_with_genes$cpg_id]

# Check the result
head(beta_matrix_with_genes)
